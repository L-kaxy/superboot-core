		...
		try {
			if ("null".equals(obj)) {
				...
				if (castClass.isAssignableFrom(Timestamp.class)) {
					final Date date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(String.valueOf(obj));
					castParm = new Timestamp(date.getTime());
				} else if (castClass.isAssignableFrom(String.class)) {
					castParm = obj;
				} else {
					castParm = castClass.getMethod("valueOf", String.class).invoke(castClass, obj.toString());
				}
			}
	...
	private static Object castPoParam(final Object obj) {
		...
		try {
			...
			if (objClass.isAssignableFrom(Timestamp.class)) {
				castParm = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(obj);
			...
			if (tempList.size() > 0) {
				...
				for (final Object key : voMap.keySet()) {
					...
					if (value != null && !value.getClass().isAssignableFrom(Class.class) && !"".equals(value)) {
						poMap.getWriteMethod(keyName).invoke(poObj, castVoParam(value, poMap.getType(keyName)));
					}
				}
			}
		...
		try {
			...
			for (final Object key : poMap.keySet()) {
				...
				if (value != null && !value.getClass().isAssignableFrom(Class.class)) {
					voMap.getWriteMethod(keyName).invoke(this, castPoParam(value));
				}
			}
		} catch (final Exception e) {
			e.printStackTrace();
